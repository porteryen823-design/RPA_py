...
try:
    ...
    print("警告: gRPC 未安裝或 proto 文件不存在，gRPC 功能將無法使用")
...
def create_grpc_stub():
    ...
    try:
        ...
        stub = screen_tools_pb2_grpc.ScreenToolsServiceStub(channel)
        return stub
    except Exception as e:
        print(f"創建 gRPC stub 失敗: {e}")
        return None
...
def screen_tools():
    ...
    action_coord_x = None  # action_coord 的 X 座標
    action_coord_y = None  # action_coord 的 Y 座標
    ...
    def show_context_menu(event):
        ...
        menu.add_command(label="17. 存 action_coord", command=lambda: save_action_coord(event))
        if PYQT5_AVAILABLE:
        ...
        menu.add_command(label="14. 傳送資料", command=send_data)
        menu.add_separator()

        # 創建滑鼠動作子選單
        mouse_submenu = tk.Menu(menu, tearoff=0)
        mouse_submenu.add_command(label="1. 移動到滑鼠位置 (coord_x,coord_y)", command=lambda: add_mouse_action("moveTo"))
        mouse_submenu.add_command(label="2. 在目前位置Click", command=lambda: add_mouse_action("click"))
        menu.add_cascade(label="15. mouse_action", menu=mouse_submenu)

        # 創建鍵盤動作子選單
        keyboard_submenu = tk.Menu(menu, tearoff=0)
        keyboard_submenu.add_command(label="1. 點選目前位置 (coord_x,coord_y)", command=lambda: add_keyboard_action("click"))
        keyboard_submenu.add_command(label="2. 目前位置全選並清除", command=lambda: add_keyboard_action("select_all_clear"))
        keyboard_submenu.add_command(label="3. 目前位置貼上文字", command=lambda: add_keyboard_action("paste_text"))
        menu.add_cascade(label="16. keyboard_action", menu=keyboard_submenu)
        ...
        if GRPC_AVAILABLE:
            ...
            if stub:
                try:
                    ...
                    # 呼叫 gRPC 服務
                    response = stub.SaveCoordinates(request)
                    print(f"gRPC 儲存座標成功: {response.message}")
                except Exception as e:
                    print(f"gRPC 儲存座標失敗: {e}")
            else:
                print("無法創建 gRPC stub")
        else:
            print("gRPC 不可用，無法傳送座標")
    ...
    class MouseSettingsWindow(QMainWindow):
        ...
        def initUI(self):
            self.setWindowTitle('滑鼠動作設定 (gRPC Client)')
    ...
    def add_mouse_action(action_type):
        ...
        try:
            ...
            if action_coord_x is not None and action_coord_y is not None:
                ...
                print("沒有可用的 action_coord 座標，請先點選右鍵設定座標")
                return

            # 透過 gRPC 傳送滑鼠動作到 server
            if GRPC_AVAILABLE:
                stub = create_grpc_stub()
                if stub:
                    try:
                        # 創建滑鼠動作請求
                        request = screen_tools_pb2.MouseActionRequest(
                            action_type=action_type,
                            x=x,
                            y=y,
                            duration=0.2,
                            button="left" if action_type == "click" else ""
                        )

                        # 呼叫 gRPC 服務
                        response = stub.ExecuteMouseAction(request)
                        print(f"gRPC 滑鼠動作 {action_type} 成功: {response.message}")
                    except Exception as e:
                        print(f"gRPC 滑鼠動作失敗: {e}")
                else:
                    print("無法創建 gRPC stub")
            else:
                print("gRPC 不可用，無法傳送滑鼠動作")

        except Exception as e:
            print(f"添加滑鼠動作時發生錯誤: {e}")
            import traceback
            traceback.print_exc()
    ...
    def add_keyboard_action(action_type):
        ...
        try:
            ...
            # 透過 gRPC 傳送鍵盤動作到 server
            if GRPC_AVAILABLE:
                stub = create_grpc_stub()
                if stub:
                    try:
                        # 創建鍵盤動作請求 (使用 SyncScreenData，因為 proto 中沒有專門的鍵盤動作)
                        coordinate = screen_tools_pb2.Coordinate(x=x, y=y, label=f"keyboard_{action_type}")
                        request = screen_tools_pb2.ScreenDataRequest(
                            data_type="add_step",
                            coordinates=[coordinate]
                        )

                        # 呼叫 gRPC 服務
                        response = stub.SyncScreenData(request)
                        print(f"gRPC 鍵盤動作 {action_type} 成功: {response.message}")
                    except Exception as e:
                        print(f"gRPC 鍵盤動作失敗: {e}")
                else:
                    print("無法創建 gRPC stub")
            else:
                print("gRPC 不可用，無法傳送鍵盤動作")

        except Exception as e:
            print(f"添加鍵盤動作時發生錯誤: {e}")
            import traceback
            traceback.print_exc()
